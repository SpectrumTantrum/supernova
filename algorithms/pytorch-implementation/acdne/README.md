...
PyTorch implementation of **Adversarial Deep Network Embedding for
...
| `layers.py`       | `FeatureExtractor` (Eqs. 1-2), `EmbeddingModule` (Eq. 4 concat fusion), `GradientReversal`, `NodeClassifier` (Eq. 6), `DomainDiscriminator` (Eq. 9) |
...
Eq. 12 and the Ganin & Lempitsky 2015 DANN trick exactly. Don't refactor to
two separate optimisers — it works as written.
...
- A PyG / DGL wrapping. The implementation uses dense (n × n) PPMI
