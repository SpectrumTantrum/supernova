"""End-to-end MHGL on the real Amazon Photo co-purchase graph.

Mirrors the printing layout of ``mhgl/example.py`` (synthetic SBM smoke test)
but loads ``AttributedNetwork`` from this folder's ``loader.load_amazon_photo``.

Run:
    cd algorithms/pytorch-implementation/mhgl/datasets/amazon_photo
    python fetch.py --small
    python run_example.py            # full run, epochs=80
    python run_example.py --smoke    # epochs=10, gates only on no-exception

Smoke-test contract
-------------------
``--smoke`` only requires that the pipeline completes without exception and
the train loss trends down. Real co-purchase data is much noisier than the
synthetic SBM, so per-run AUC under 10 epochs is too volatile to gate on.

Full runs additionally check:
    1. Train loss trends down.
    2. ROC-AUC on V_test >= AUC_FLOOR (default 0.65, soft floor).
    3. Welch's t-test on V_test scores: anomaly > normal, p < 0.05.
    4. Welch's t-test restricted to {normals, unseen-only}: p < 0.05 with
       positive mean-gap (gates the unseen-detection claim).
"""
...
# Put parent mhgl/ on sys.path for ``model``/``data`` imports.
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2]))
from data import ANOM_UNSEEN  # noqa: E402
from model import MHGL, MHGLConfig  # noqa: E402

from loader import load_amazon_photo  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--smoke", action="store_true",
                    help="Fast pipeline check: epochs=10, gate only on no-exception + loss-down.")
    ap.add_argument("--hidden-dims", type=_parse_dims, default=(64, 32, 16, 16))
    ap.add_argument("--k-normal", type=int, default=3)
    ap.add_argument("--sigma", type=float, default=1.0)
    ap.add_argument("--radius-quantile", type=float, default=0.5)
    ...
    print("=" * 64)
    print("MHGL — Amazon Photo (real co-purchase graph, rare-class anomaly protocol)")
    print("=" * 64)

    print("\n[1/4] Loading Amazon Photo…")
    net = load_amazon_photo(seed=args.seed)
    ...
    if args.smoke:
        if loss_down:
            print("\n✓ PASS (smoke) — pipeline ran end-to-end and train loss trended down.")
            return 0
        print(
            f"\n✗ FAIL (smoke) — train loss did not trend down "
    ...
    if loss_down and auc_ok and gap_ok and p_all_ok and p_unseen_ok:
        print("\n✓ PASS — MHGL distinguishes seen + unseen anomalies on Amazon Photo.")
        return 0
    ...
    if not loss_down:
        ...
        reason = "no unseen anomalies in V_test — loader misconfigured"
    elif p_unseen >= 0.05:
