...
def build_skipgram_pairs(
    ...
    *,
    ...
    """
    if time_threshold_min < 0:
        raise ValueError(f"time_threshold_min must be >= 0, got {time_threshold_min}")
    ...
    for idxs in by_cat.values():
        # Sort by D time and duplicate the day to make the midnight boundary a
        # normal sliding-window case, e.g. 23:55 pairs with 00:05.
        idxs.sort(key=lambda i: d_events[i].time_min)
        L = len(idxs)
        if L < 2:
            continue
        times = [d_events[i].time_min % day_minutes for i in idxs]
        idxs_ext = idxs + idxs
        times_ext = times + [t + day_minutes for t in times]
        for i in range(L):
            t_i = times_ext[i]
            k = i + 1
            while k < i + L:
                if times_ext[k] - t_i > time_threshold_min:
                    break
                a, b = o_events[idxs[i]].event_idx, o_events[idxs_ext[k]].event_idx
                pairs.append((a, b))
                pairs.append((b, a))
                k += 1
    return pairs
