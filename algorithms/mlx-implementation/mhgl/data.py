...
def build_normalized_adj(edges: np.ndarray, n: int) -> np.ndarray:
    """Dense A_hat = D^{-1/2}(A+I)D^{-1/2} for MLX GCN propagation."""
    ...
    if edges.size:
        ...
        A[j, i] = 1.0
    deg = A.sum(axis=1)
    inv_sqrt = np.where(deg > 0, deg ** -0.5, 0.0).astype(np.float32)
